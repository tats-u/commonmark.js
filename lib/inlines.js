...
var isInlineSpace = function(c) {
    return c === 0x20 || c === 9 || c === 0xa || c === 0xd;
};

var trimInlineWhitespace = function(str) {
    var start = 0;
    var end = str.length - 1;

    while (start <= end && isInlineSpace(str.charCodeAt(start))) {
        start++;
    }

    while (end >= start && isInlineSpace(str.charCodeAt(end))) {
        end--;
    }
...
var parseInlines = function(block) {
    // String.prototype.trim() removes non-ASCII whitespaces, vertical tab, form feed and so on.
    ...
    // Keep the historical behavior of trimming only ASCII whitespace.
    this.subject = trimInlineWhitespace(block._string_content);
    this.pos = 0;
